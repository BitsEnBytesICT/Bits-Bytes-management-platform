...
export type Result<a, b> = (
    | { kind: "error", key: keyof a, errorMSG: IError }
    | { kind: "success", key: keyof a, result?: b });
...
export const pipe = <a extends object, T extends AllKeysValidators<a>[]>(data: a, ...args: T & HasAllKeys<a, T>): Result<a, a[keyof a]>[] => {
    ...
    (args as Array<[keyof a, Fun<any, boolean>, string]>).forEach((func) => {
        ...
        try {
            if (validator(data[key])) results.push({ kind: "success", result: data[key], key });
            else results.push({ kind: "error", key, errorMSG: { date: new Date(), errorMSG } });
        } catch (error) {
            results.push({ kind: "error", key, errorMSG: { date: new Date(), errorMSG: error } });
        }
    });
    return results;
};
...
//const results = pipe(user, ["name", validateName, "Name is empty"], ["age", validateAge, "Age is negative"]);
...
//if (results.find((r) => r.kind == "error")) console.log("error found!!!"); //check for any error